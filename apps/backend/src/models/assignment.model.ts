...
/**
 * Embedded file reference — mirrors AssignmentFileRef in @learnova/types.
 * `id` is an application-generated string so refs stay stable when copied
 * between an assignment, a submission and a comment.
 */
...
const assignmentSchema = new Schema(
  {
    ...
    rubricId: { type: Schema.Types.ObjectId, ref: 'AssignmentRubric', default: null, index: true },
...
assignmentSchema.index({ institutionId: 1, courseId: 1, status: 1 });
assignmentSchema.index({ institutionId: 1, status: 1, dueDate: 1 });
assignmentSchema.index({ institutionId: 1, createdBy: 1, status: 1 });
assignmentSchema.index({ institutionId: 1, moduleId: 1 });
assignmentSchema.index({ institutionId: 1, lessonId: 1 });
