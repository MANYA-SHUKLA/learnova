...
import { enrollmentApi, useEnrollmentImportMutation } from '@/features/enrollment';
import { ApiClientError } from '@/lib/api/client';
import { Link, useRouter } from '@/lib/i18n/routing';
import type { EnrollmentImportPreview } from '@learnova/types';
...
export default function EnrollmentImportPage() {
  const t = useTranslations('dashboard.institution.enrollments.import');
  const tCommon = useTranslations('common');
  const router = useRouter();
  const importMutation = useEnrollmentImportMutation();
  ...
  const onFile = async (file: File | null) => {
    ...
    setRows(parsed);
    try {
      const result = await enrollmentApi.previewImport(parsed);
      setPreview(result);
    } catch (err) {
  ...
  const onImport = async () => {
    ...
    try {
      ...
      router.push(APP_ROUTES.INSTITUTION_ENROLLMENTS);
    } catch (err) {
          ...
          <CardContent className="space-y-4">
            ...
            {preview ? (
              <div className="space-y-2 rounded-xl border border-border bg-muted/20 p-4 text-sm">
                ...
                <p>
                  Invalid: <strong>{preview.invalidRows}</strong>
                </p>
                <p>
                  Duplicates: <strong>{preview.duplicates}</strong>
                </p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild variant="outline">
                <Link href={APP_ROUTES.INSTITUTION_ENROLLMENTS}>{tCommon('cancel')}</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
