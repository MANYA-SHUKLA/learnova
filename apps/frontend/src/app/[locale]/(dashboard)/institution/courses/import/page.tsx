...
import { courseApi, useCourseImportMutation } from '@/features/course';
...
export default function CourseImportPage() {
  const t = useTranslations('dashboard.institution.courses.import');
  ...
  const onFile = async (file: File | null) => {
    ...
    setRows(parsed);
    try {
      const result = await courseApi.previewImport(parsed);
      setPreview(result);
    } catch (err) {
  ...
  const onImport = async () => {
    ...
    try {
      ...
      router.push(APP_ROUTES.INSTITUTION_COURSES);
    } catch (err) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild variant="outline">
                <Link href={APP_ROUTES.INSTITUTION_COURSES}>{tCommon('cancel')}</Link>
              </Button>
            </div>
          </CardContent>
        </Card>
